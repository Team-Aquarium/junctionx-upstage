...
try {
  ...
  await waitWatching(
    page,
    async () =>
      (await page.getByText(/Fit \d+/).count()) > 0 ||
      (await page.getByText(/적합도/).count()) > 0,
    180000,
  );
  await page.evaluate(() => {
  ...
  await page.getByPlaceholder("Search by title, organizer, or field…").fill("AI");
  await hold(page, 1000);
  await page.getByRole("button", { name: "Eligible only" }).click();
  await hold(page, 1200);
  ...
  await waitWatching(
    page,
    async () =>
      (await page.getByText(/document parse|information extract|studio agent/i).count()) > 0,
    200000,
  );
...
const rawMp4 = path.join(outDir, "moabora-demo-1x.mp4");
const encode = spawnSync(
  "ffmpeg",
  ["-y", "-i", rawPath, "-c:v", "libx264", "-pix_fmt", "yuv420p", "-movflags", "+faststart", rawMp4],
  { stdio: "inherit" },
);
if (encode.status !== 0) {
...
const duration1x = Number.parseFloat(probe.stdout.trim());
await composeNarratedVideo({
  rawMp4,
  duration1x,
  markers,
  outDir,
...
console.log(path.join(outDir, "moabora-demo.mp4"));
...
async function composeNarratedVideo({ rawMp4, duration1x, markers: marks, outDir: dir }) {
  const video2x = path.join(dir, "moabora-demo-2x.mp4");
  const sped = spawnSync(
    "ffmpeg",
    [
      "-y",
      "-i",
  ...
  if (sped.status !== 0) {
    throw new Error("ffmpeg failed to speed up the video");
  }

  const remixed = spawnSync("node", [path.join(root, "scripts/remix-demo.mjs")], {
    stdio: "inherit",
  });
  if (remixed.status !== 0) {
    throw new Error("failed to remix narration and subtitles");
  }
