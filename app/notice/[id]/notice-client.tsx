...
export function NoticeClient({ id }: { id: string }) {
  const { t, locale, setLocale } = useI18n();
  const [item, setItem] = useState<AnnouncementWithMatch | null>(() =>
    id === DEMO_NOTICE_ID ? createDemoNotice(t) : null,
  );
  const [loading, setLoading] = useState(() => id !== DEMO_NOTICE_ID);
  const [checked, setChecked] = useState<Set<number>>(new Set());
  const [rec, setRec] = useState<{ score: number; reason: string } | null>(() =>
    id === DEMO_NOTICE_ID ? createDemoRecommendation(t) : null,
  );
